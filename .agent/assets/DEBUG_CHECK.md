...
- [ ] サンプルフォルダ（`example/`）が存在する場合、その中に README.md が存在するか
- [ ] サンプルフォルダ（`example/`）の README.md が大元の README.md を適切に参照しているか
- [ ] 大元の README.md とサンプルフォルダ（`example/`）の README.md の内容が重複せず、適切に分割されているか
...
- [ ] 直接コード内にAPIキーやパスワードなどの機密情報が記載されていないか（`.env`は許可）
...
- [✅/❌/⚠️] 大元の README.md が存在し、適切に構成されているか
  - 問題点と改善提案（問題がある場合）
- [✅/❌/⚠️] サンプルフォルダ（`example/`）が存在する場合、その中に README.md が存在するか
  - 問題点と改善提案（問題がある場合）
- ...（以下同様）
...
2. サンプルフォルダ（`example/`）が存在する場合、その中に README.md があるか確認し、なければ作成を提案する
3. サンプルフォルダ（`example/`）の README.md が大元の README.md を適切に参照し、重複を避けているか確認する
